...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val numberOfNullCells = board.getAllCells().filter { board[it] == null }

        return if (numberOfNullCells.isEmpty()){
            null
        } else{
            val index = Random.nextInt(numberOfNullCells.size)
            numberOfNullCells[index] to generateRandomStartValue()
        }
    }
}